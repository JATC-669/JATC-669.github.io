...
function setInitDates() {
	...
	var backDateValue = dat.addDays(0);
	backDate.value = backDateValue.getFullYear() + '/' + (backDateValue.getMonth() + 1) + '/' + backDateValue.getDate();
	
	if (backDateValue.addDays(3) === 6) {
		newDueDate = backDateValue.addDays(5)
	}

	else if (backDateValue.addDays(3) === 0) {
		newDueDate = backDateValue.addDays(5)
	}

	else {
		newDueDate = backDateValue.addDays(3)
	}
	dueDate.value = newDueDate.getFullYear() + '/' + (newDueDate.getMonth() + 1) + '/' + newDueDate.getDate();
	alert(newDueDate)
...
function createGraderInfo() {
	...
	graderLessonsReceived.innerHTML = backDate.value;
	// worked graderLessonsReceived.innerHTML = (entDate.getMonth() + 1) + '/' + entDate.getDate() + '/' + entDate.getFullYear();
...
function addEntry() {
	...
	if (stuId === '') {
		...
		else if (stuId.length != 6) {document.getElementById('stuId').style.border = '1px solid red';}
...
function run(entId, stuId, fName, lName, courseName, lessons) {
	...
	// worked cell1.innerHTML = (entDate.getMonth() + 1) + '/' + entDate.getDate() + '/' + entDate.getFullYear();

	cell1.innerHTML = backDate.value;
	cell1.className = "hide-for-print";
	cell2.innerHTML = entDate.getTime(); // Hiding this with css classes
...
function deleteRow(t) {
    ...
    // Need to build this still: createGraderInfo();
}
